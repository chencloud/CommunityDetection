...
//				double curError = AdjminusMatrix(UTimesUTranspose(newU));
				tempU = newU;
			}
	...
	private double[][] UpateU(double[][] olderU, double[][] temp1,
		...
		for(int irow=0;irow<olderU.length;irow++)
		{
			for(int jcol=0;jcol<olderU[irow].length;jcol++)
			{
				olderU[irow][jcol] = olderU[irow][jcol]*temp1[irow][jcol]/(this.eps+temp3[irow][jcol]);
			}
		}
		return olderU;
	}
